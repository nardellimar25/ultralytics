...
// helper to write bgrx
static __device__ __forceinline__ void write_bgrx(
    unsigned char* img,
    int W, int H,
    int x, int y,
    unsigned char b, unsigned char g, unsigned char r,
...
// NEW: same function and kernel but for bgrx
__global__ void k_sbs_downscale_nn_bgrx_u8(
    const unsigned char* __restrict__ in_bgr_batched,
    int cap_w, int cap_h,
    int out_w, int out_h,
    int num_cams,
    unsigned char* __restrict__ out_sbs_bgrx
) {
    ...
    const int out_W = out_w * num_cams;
    const int ox = cam * out_w + x;
    const int dst_idx = (y * out_W + ox) * 4;

    out_sbs_bgrx[dst_idx + 0] = b;
    out_sbs_bgrx[dst_idx + 1] = g;
    out_sbs_bgrx[dst_idx + 2] = r;
    out_sbs_bgrx[dst_idx + 3] = 255;  // X
}
...
void launch_sbs_downscale_bgrx_u8(
    ...
    if (!d_bgr_undistorted_batched || !d_sbs_small_bgrx_u8) return;
    if (num_cameras <= 0 || cap_width <= 0 || cap_height <= 0) return;
    if (out_w_small <= 0 || out_h_small <= 0) return;
    ...
    k_sbs_downscale_nn_bgrx_u8<<<grid, block, 0, stream>>>(
        d_bgr_undistorted_batched,
        cap_width, cap_height,
        out_w_small, out_h_small,
        num_cameras,
    ...
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        std::fprintf(stderr, "k_sbs_downscale_nn_bgrx_u8 launch failed: %s\n", cudaGetErrorString(err));
    }
}


// NEW: same function and kernel but for bgrx
__global__ void k_draw_boxes_sbs_bgrx_u8(
    unsigned char* __restrict__ sbs_bgrx,
    int sbs_W, int sbs_H,
    int num_cams,
    const ActionVis* __restrict__ dets,
    int N,
    ...
    unsigned char b = 0, g = 255, r = 0;
    if (cls == 1) { b = 0; g = 0;   r = 255; }
    if (cls == 2) { b = 0; g = 255; r = 255; }

    const int t = 1;

    for (int x = x1 + threadIdx.x; x <= x2; x += blockDim.x) {
        for (int dy = 0; dy < t; ++dy) {
            write_bgrx(sbs_bgrx, sbs_W, sbs_H, x, y1 + dy, b, g, r, 255);
            write_bgrx(sbs_bgrx, sbs_W, sbs_H, x, y2 - dy, b, g, r, 255);
        }
    }

    for (int y = y1 + threadIdx.x; y <= y2; y += blockDim.x) {
        for (int dx = 0; dx < t; ++dx) {
            write_bgrx(sbs_bgrx, sbs_W, sbs_H, x1 + dx, y, b, g, r, 255);
            write_bgrx(sbs_bgrx, sbs_W, sbs_H, x2 - dx, y, b, g, r, 255);
        }
    }
}
...
void launch_draw_boxes_sbs_bgrx_u8(
    ...
    k_draw_boxes_sbs_bgrx_u8<<<blocks, threads, 0, stream>>>(
        d_sbs_small_bgrx_u8,
        out_w_small_sbs,
        out_h_small,
        num_cameras,
    ...
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        std::fprintf(stderr, "k_draw_boxes_sbs_bgrx_u8 launch failed: %s\n", cudaGetErrorString(err));
    }
}
