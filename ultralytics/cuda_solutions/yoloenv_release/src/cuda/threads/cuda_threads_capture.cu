...
// TODO : move to a threads_shared_state.cu file
cv::Mat frame_back;
cv::Mat frame_front;
...
void multistream_frame_capture_thread(
    ...
    nvtxRangePush("MultiStreamFrameCaptureThread");
    ...
    std::vector<int> cam_probe_cooldown(num_cameras, 0);

    // if fails for a whole second, disable pull for 3 seconds
    // TODO : make these constants config params
    const int MAX_CONSEC_FAIL = 30;  
    const int PROBE_COOLDOWN_FRAMES = 90;
    const GstClockTime PROBE_TIMEOUT_NS = 200 * GST_MSECOND;
    const GstClockTime PULL_TIMEOUT_NS = 5 * GST_MSECOND; 
    ...
    while (keep_running) {
        ...
        for (int cam = 0; cam < num_cameras; ++cam) {
            ...
            // TODO : test use gst_app_sink_try_pull_sample() with timeout
            ...
            // NEW: use try_pull_sample with timeout to avoid blocking indefinitely
            GstSample* sample = gst_app_sink_try_pull_sample(GST_APP_SINK(sink), PULL_TIMEOUT_NS);

            if (!sample) {
                // No new frame within timeout -> skip this cam this cycle (keep last image)
                continue;
            }
        ...
        // NEW: Publish a new frame id and wake consumers (inference + debug-vis)
    ...
    nvtxRangePop(); // MultiStreamFrameCaptureThread
}
